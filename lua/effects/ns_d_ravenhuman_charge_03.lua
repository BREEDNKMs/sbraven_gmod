...
function EFFECT:Init(data) 
	...
	self.LifeTime = self.LifeTime != 0 and self.LifeTime or 0.50 
...
function EFFECT:NE_SpriteM_Init(data) 
	local SpawnParticleCumuls = function(pos) 
		...
		for i = 1, 50 do
			...
			local life = math.Rand(self.LifeTime*0.5, self.LifeTime)
			...
			local mid = (spawnPos + self:GetRenderPos()) * 0.5
			-- offset magnitude proportional to distance (so long shots bend more)
			local dist = spawnPos:Distance(self:GetRenderPos())
			...
			p:SetThinkFunction(function(part)
				...
				local newPos = QuadraticBezier(part.SpawnPosition, part.BendPoint, self:GetRenderPos(), te)
...
function EFFECT:Think() 
	local now = CurTime()
	local Cycle = math.Clamp((now - self.CreationTime) / self.LifeTime, 0, 1) 
